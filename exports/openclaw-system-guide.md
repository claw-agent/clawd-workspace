...
This system turns OpenClaw from a chatbot into a **persistent AI partner** — one that remembers context across sessions, runs scheduled tasks, manages its own memory, and operates proactively. The key insight: **files are memory, sessions are ephemeral.** Everything important lives in markdown files the agent reads on startup.
...
**Humor is allowed.** Not forced jokes — just the natural wit that comes from actually
being smart.
...
**If you modify SOUL.md, IDENTITY.md, or AGENTS.md — always tell the user.** These
...
The entire system is built on one principle: **files are the source of truth.** Sessions are ephemeral — they get compacted, they crash, they expire. But files persist. Every important decision, every piece of context, every learned lesson goes into a file.
